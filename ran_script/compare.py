...
# Function to get product-level data for Jeju
def get_jeju_product_data(dfs):
    print("\n===== Extracting Jeju Product-Level Data =====")

    # Extract data specific to Jeju from each dataset
    jeju_data = {}

    # Process df_volume data
    jeju_volume = dfs['df_volume'][dfs['df_volume']['Location'] == 'Jeju'].copy()
    if not jeju_volume.empty:
        print(f"Found {len(jeju_volume)} products in df_volume for Jeju")

        # Identify volume column
        volume_col = 'DF_Vol' if 'DF_Vol' in jeju_volume.columns else '$current_year Volume'

        # Split into PMI and competitor products
        pmi_volume = jeju_volume[jeju_volume['TMO'] == 'PMI'].copy()
        comp_volume = jeju_volume[jeju_volume['TMO'] != 'PMI'].copy()

        # Calculate total volumes
        total_volume = jeju_volume[volume_col].sum()
        ...
        print(f"  PMI market share in Jeju: {pmi_market_share * 100:.2f}%")
        print(f"  Volume column identified: {volume_col}")
    else:
        print("No Jeju data found in df_volume")

    # Process cat_a_df_vols data
    jeju_cat_a = dfs['cat_a_df_vols'][dfs['cat_a_df_vols']['Location'] == 'Jeju'].copy()
    if not jeju_cat_a.empty:
        print(f"Found {len(jeju_cat_a)} products in cat_a_df_vols for Jeju")

        # Identify volume column
        volume_cols = [col for col in jeju_cat_a.columns if 'volume' in col.lower() and '$current' in col.lower()]
        volume_col = volume_cols[0] if volume_cols else None

        if volume_col:
            # Split into PMI and competitor products
            pmi_cat_a = jeju_cat_a[jeju_cat_a['TMO'] == 'PMI'].copy()
            comp_cat_a = jeju_cat_a[jeju_cat_a['TMO'] != 'PMI'].copy()

            # Calculate total volumes
            total_volume = jeju_cat_a[volume_col].sum()
            ...
            jeju_data['cat_a_df_vols'] = {
                'all_products': jeju_cat_a,
            ...
            print(f"  PMI market share in Jeju from cat_a_df_vols: {pmi_market_share * 100:.2f}%")
            print(f"  Volume column identified: {volume_col}")
        else:
            print("  No volume column found in cat_a_df_vols for Jeju")
    else:
        print("No Jeju data found in cat_a_df_vols")

    # Process market_mix data
    jeju_market_mix = dfs['market_mix'][dfs['market_mix']['Location'] == 'Jeju'].copy()
    if not jeju_market_mix.empty:
        print(f"Found {len(jeju_market_mix)} products in market_mix for Jeju")
        jeju_data['market_mix'] = jeju_market_mix
    else:
        print("No Jeju data found in market_mix")

    # Process PARIS_Output data
    jeju_paris = dfs['paris_output'][dfs['paris_output']['Location'] == 'Jeju'].copy()
    if not jeju_paris.empty:
        print(f"Found {len(jeju_paris)} entries in PARIS_Output for Jeju")
        jeju_data['paris_output'] = jeju_paris
    else:
        print("No Jeju data found in PARIS_Output")

    # Process passenger data
    jeju_pax = dfs['pax_data'][dfs['pax_data']['Market'] == 'Jeju'].copy()
    if not jeju_pax.empty:
        print(f"Found {len(jeju_pax)} entries in pax_data for Jeju")
        jeju_data['pax_data'] = jeju_pax
    else:
        print("No Jeju data found in pax_data")

    return jeju_data


# Get product data for Jeju
jeju_data = get_jeju_product_data(dfs)


# Merge attribute data with product data
def merge_product_attributes(jeju_data):
    ...
    if 'df_volume' in jeju_data:
        primary_source = 'df_volume'
        primary_products = jeju_data['df_volume']['all_products'].copy()
        volume_col = jeju_data['df_volume']['volume_col']
        print(f"Using df_volume as primary source with {len(primary_products)} products")
    elif 'cat_a_df_vols' in jeju_data:
        primary_source = 'cat_a_df_vols'
        primary_products = jeju_data['cat_a_df_vols']['all_products'].copy()
        volume_col = jeju_data['cat_a_df_vols']['volume_col']
        print(f"Using cat_a_df_vols as primary source with {len(primary_products)} products")
    else:
        print("No product data available for Jeju")
        return None

    # Merge with attribute data from market_mix or selma_df_map
    product_data = primary_products.copy()

    if 'market_mix' in jeju_data and 'CR_BrandId' in product_data.columns:
        market_mix = jeju_data['market_mix']
    ...
    if primary_source == 'df_volume':
        ...
        merged_data['pmi_share'] = jeju_data['df_volume']['pmi_share']
    elif primary_source == 'cat_a_df_vols':
        merged_data['pmi_products'] = product_data[product_data['TMO'] == 'PMI'].copy()
        merged_data['comp_products'] = product_data[product_data['TMO'] != 'PMI'].copy()
        merged_data['pmi_share'] = jeju_data['cat_a_df_vols']['pmi_share']

    # Add PARIS_Output data if available
    if 'paris_output' in jeju_data:
        merged_data['paris_output'] = jeju_data['paris_output']

    return merged_data


# Merge product attributes
jeju_merged = merge_product_attributes(jeju_data)
...
jeju_top_90 = identify_top_90_percent(jeju_merged)
...
jeju_attr_dist = analyze_attribute_distribution(jeju_top_90, jeju_merged)
...
jeju_top_products = identify_top_products_by_attribute(jeju_top_90, jeju_attr_dist)
...
def generate_portfolio_recommendations(top_90_data, attr_dist, top_products):
    ...
    print(f"Current PMI market share in Jeju: {pmi_share * 100:.2f}%")

    if pmi_share < 0.20:
        market_strategy = "Expansion"
        print("\nRecommended Overall Strategy: EXPANSION")
        print(
            "PMI has significant room for growth in the Jeju market. Focus on introducing new SKUs in key segments and expanding distribution of existing SKUs.")
    elif pmi_share < 0.50:
        market_strategy = "Balanced Growth"
        print("\nRecommended Overall Strategy: BALANCED GROWTH")
        print(
            "PMI has a moderate presence in the Jeju market. Focus on both expanding in key segments and optimizing the existing portfolio for efficiency.")
    else:
        market_strategy = "Optimization/Defense"
        print("\nRecommended Overall Strategy: OPTIMIZATION/DEFENSE")
        print(
            "PMI has a strong presence in the Jeju market. Focus on defending market share, optimizing the portfolio, and rationalizing underperforming SKUs.")
    ...
    print(f"1. Overall Portfolio Strategy for Jeju: {market_strategy.upper()}")
...
jeju_recommendations = generate_portfolio_recommendations(jeju_top_90, jeju_attr_dist, jeju_top_products)


# Function to analyze passenger mix and its influence on the ideal portfolio
def analyze_passenger_mix(jeju_data):
    print("\n===== Passenger Mix Analysis =====")

    if 'pax_data' not in jeju_data:
        print("No passenger data available for Jeju")
        return None

    pax_data = jeju_data['pax_data']

    if not pax_data.empty:
        print(f"Found {len(pax_data)} passenger data entries for Jeju")
...
passenger_analysis = analyze_passenger_mix(jeju_data)
            ...
            if 'combined_dist' in data:
                ...
                if 'PMI_Volume_Percentage' in combined_dist.columns and 'Ideal_Percentage' in combined_dist.columns:
                    try:
                        ...
                        plt.title(f'Jeju {attr} Distribution: Market vs. PMI vs. Ideal', fontsize=14)
                        ...
                        # Save the figure
                        plt.savefig(f"Jeju_{attr}_distribution.png")
                        print(f"Plot saved as Jeju_{attr}_distribution.png")
                        ...
                        plt.title(f'Jeju {attr} Gaps: PMI vs. Ideal', fontsize=14)
                        ...
                        # Save the figure
                        plt.savefig(f"Jeju_{attr}_gaps.png")
                        print(f"Plot saved as Jeju_{attr}_gaps.png")
    ...
    if passenger_analysis and 'nationality_dist' in passenger_analysis:
        try:
            ...
            if len(nationality_dist) > 0:
                ...
                plt.ylabel('Percentage (%)', fontsize=12)
                plt.title('Jeju Passenger Distribution by Top 10 Nationalities', fontsize=14)
                plt.xticks(rotation=45)
                plt.grid(axis='y', linestyle='--', alpha=0.7)
                plt.tight_layout()

                # Save the figure
                plt.savefig("Jeju_passenger_distribution.png")
                print("Plot saved as Jeju_passenger_distribution.png")
        except Exception as e:
            print(f"Error creating passenger visualization: {e}")


# Create visualizations
create_visualizations(jeju_attr_dist, jeju_top_products, passenger_analysis)


# Export all findings to Excel for further analysis
def export_to_excel(jeju_top_90, attr_dist, top_products, passenger_analysis):
    print("\n===== Exporting Results to Excel =====")

    # Create Excel writer
    excel_path = data_dir / 'jeju_product_analysis.xlsx'
    with pd.ExcelWriter(excel_path) as writer:
        # 1. Export top 90% products
        if jeju_top_90 and 'all_products' in jeju_top_90:
            jeju_top_90['all_products'].to_excel(writer, sheet_name='Top_90pct_Products', index=False)
            print(f"Exported top 90% products ({len(jeju_top_90['all_products'])} rows)")

        # 2. Export PMI products in top 90%
        if jeju_top_90 and 'pmi_products' in jeju_top_90 and not jeju_top_90['pmi_products'].empty:
            jeju_top_90['pmi_products'].to_excel(writer, sheet_name='PMI_Products', index=False)
            print(f"Exported PMI products ({len(jeju_top_90['pmi_products'])} rows)")
        ...
        if jeju_top_90:
            summary_data.append({
                ...
                'Value': f"{jeju_top_90['pmi_share'] * 100:.2f}%"
            })
            summary_data.append({
                'Category': 'Market Share',
                'Metric': 'PMI Products Count',
                'Value': jeju_top_90['pmi_products_count']
            })
            summary_data.append({
                'Category': 'Market Share',
                'Metric': 'Competitor Products Count',
                'Value': jeju_top_90['comp_products_count']
            })
...
export_to_excel(jeju_top_90, jeju_attr_dist, jeju_top_products, passenger_analysis)
